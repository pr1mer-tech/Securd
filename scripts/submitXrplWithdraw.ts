 ...
 *   NEXT_PUBLIC_XRPL_AXELAR_GATEWAY          default: rNrjh1KGZk2jBR3wPfAQnoidtFFYQKbQn2
 *   NEXT_PUBLIC_AXELAR_DESTINATION_CHAIN        default: xrpl-evm
 *   XRPL_GMP_GAS_DROPS           GMP gas drops for inbound relay (default: 3000000)
 *   NEXT_PUBLIC_XRPL_CONFIRM_SEND            Set to "true" to actually submit
 */
...
async function main() {
    ...
    const xrplGateway = optionalEnv(
        "NEXT_PUBLIC_XRPL_AXELAR_GATEWAY",
    ...
    const confirmSend = process.env.NEXT_PUBLIC_XRPL_CONFIRM_SEND === "true";
    ...
    if (!confirmSend) {
        console.log("\nSet NEXT_PUBLIC_XRPL_CONFIRM_SEND=true to submit.");
        return;
    }
