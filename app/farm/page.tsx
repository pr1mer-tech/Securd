...
export default async function Farm({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined } }) {
    ...
    const tokenList = pools?.pools.flatMap((pool: Pool & {
        token_0: Token | null,
        token_1: Token | null
    }) => [pool.token_0, pool.token_1]).filter(token => token !== null);
    const uniqueTokenList = tokenList?.filter((token, index, self) => self.findIndex(t => t?.token_address === token?.token_address) === index) ?? [];

    const reservesInfo = await Promise.all(uniqueTokenList.map(token => tokenToReserveInfo(token, pools)) ?? []);

    return <FarmSync preReservesInfo={reservesInfo} pools={pools?.pools ?? []} chainId={chain}>
