...
// Deployed contract addresses. Env-driven so the same code targets testnet
// (defaults below) or mainnet — set the NEXT_PUBLIC_ vars per deployment/branch.
// NEXT_PUBLIC_ is required (read in the browser) and each var must be referenced
// literally so Next.js inlines it at build time.
export const ADDRESSES = {
  comptroller:
    (process.env.NEXT_PUBLIC_COMPTROLLER_ADDRESS as Address) ??
    ("0x46d364257112230022E72b086Df85a6b0f8D3F86" as Address),
  oracle:
    (process.env.NEXT_PUBLIC_ORACLE_ADDRESS as Address) ??
    ("0x517475AFaFfaE71491d9Bad598E07AAFD050Ca80" as Address),
  interestRateModel:
    (process.env.NEXT_PUBLIC_INTEREST_RATE_MODEL_ADDRESS as Address) ??
    ("0xDd31C1db90AB0b094d73E0b4c8dae2296a7d8C0d" as Address),
  proxyFactory:
    (process.env.NEXT_PUBLIC_PROXY_FACTORY_ADDRESS as Address) ??
    ("0xB7f3ECe856063F48BC3bcC7A381aE875841663aA" as Address),
  bridgeAdapter:
    (process.env.NEXT_PUBLIC_BRIDGE_ADAPTER_ADDRESS as Address) ??
    ("0x7AC8Df85448037c6fE1eD5732c6ca71060069237" as Address),
  liquidationKeeper:
    (process.env.NEXT_PUBLIC_LIQUIDATION_KEEPER_ADDRESS as Address) ??
    ("0xF87Bda7207B629789abaaCcef366Ba853BA11399" as Address),
} as const;
