...
const useGetCollateralProportions = (
    ...
    contracts: collateralInfos.flatMap((info) => [
      {
        ...
        args: [
          info.addressLP,
          collateralAmountPrice[info.addressLP]?.collateralAmount,
        ],
      },
      {
        ...collateralPriceContract,
  ...
  if (
    !data ||
    data.length < collateralInfos.length * 2 ||
    collateralInfos.length === 0
  ) {
    return {};
  }
  ...
  const amounts = Object.fromEntries(
    [...Array(Math.ceil(flatArray.length / 2))]
      .map(() => flatArray.splice(0, 2))
      .map((item, i) => {
        ...
        return [
          info.addressLP,
          {
            ...
            collateralPrice: item[1]?.[2] ?? 0n,
          },
        ];
      })
      .filter((entry): entry is [Address, any] => entry !== null),
  );
