...
const useAssetPriceOracle: (
	...
	if (!data)
		return Object.fromEntries(
			reservesInfo.map((reserve, index) => [
				reserve.symbol as keyof Coins,
				1,
			]),
	...
	const coinPrices = reservesInfo.reduce(
		(acc, reserve, index) => {
			if (
				typeof data[index].result !== "string" &&
				typeof data[index].result !== "bigint"
			) {
				acc[reserve.symbol as keyof Coins] = 1;
				return acc;
			}
			const price = weiToEth(
				BigInt(data[index].result as string | bigint),
			);
