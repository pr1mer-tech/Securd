...
export function repay(
  ...
  return async function* repayPipeline() {
    ...
    const userBalance = isEqualAddress(token0.address, selectedAsset.address)
    ...
    const debt0 =
      parseUnits(borrowBalance.borrowBalanceA.toString(), token0.decimals) -
      (isEqualAddress(selectedAsset.address, token0.address) ? amount : 0n);
    const debt1 =
      parseUnits(borrowBalance.borrowBalanceB.toString(), token1.decimals) -
      (isEqualAddress(selectedAsset.address, token1.address) ? amount : 0n);

    const adjustedPriceA =
      debt0 * BigInt(Math.round((tokensUSDPrices.tokenA ?? 0) * 1e6));
    const adjustedPriceB =
      debt1 * BigInt(Math.round((tokensUSDPrices.tokenB ?? 0) * 1e6));
