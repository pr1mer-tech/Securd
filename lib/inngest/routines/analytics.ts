	...
	async ({ step }) => {
		...
		await step.run("get-pair-data-and-update-db", async () => {
			...
			const _pairData = pairs.map(async (pair) => {
				try {
					const data = await getPairDayData(
						pair.pool_address,
						nbDays,
					);
					...
					console.warn(
						`Skipping pair ${pair.pool_address} due to error:`,
						error,
					);
					return null;
				}
			...
			await db.transaction(async (tx) => {
				...
				const count = pairData.reduce(
					(acc, data) => acc + data.updatedData.length,
					0,
				);
				...
				console.log(
					`Inserted ${analyticsChunks.length} analytics records`,
				);
