...
export const columns: ColumnDef<ReserveInfo>[] = [
	...
	},
	{
		...
		sortingFn: (a, b) => {
			...
			const depositBalanceA =
				bigIntToDecimal(_depositBalanceA, a.original.decimals) ?? 0;
			const depositBalanceB =
				bigIntToDecimal(_depositBalanceB, b.original.decimals) ?? 0;
		...
		cell: ({ row }) => {
			...
			const depositBalance = bigIntToDecimal(
				_depositBalance,
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			const globalDeposit = bigIntToDecimal(
				row.getValue("deposit") as bigint,
				row.original.decimals,
			);
			const price = coinPrices[row.original.symbol as keyof Coins];
			return (
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			const globalInterest = bigIntToDecimal(
				_globalInterest,
				row.original.decimals,
			);
			const price = coinPrices[row.original.symbol as keyof Coins];
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			const liquidity = bigIntToDecimal(
				_liquidity,
				row.original.decimals,
			);
			const price = coinPrices[row.original.symbol as keyof Coins];
	...
	},
	{
		...
		cell: ({ row }) => {
			return (
				<div className="text-xl font-bold">
					{toFormattedPercentage(row.getValue("utilization"), 1)}
				</div>
			);
		},
	},
	{
		...
		cell: ({ row }) => {
			return (
				<div className="text-xl font-bold">
					{toFormattedPercentage(row.getValue("savingsApy"), 1)}
				</div>
			);
		},
	},
...
export default function AllAccounts() {
	...
	return (
		<Tabs
			value={mode}
			onValueChange={(v) => setMode(v as "table" | "grid")}
		>
			<div className="mt-4 mb-16">
				<div className="flex flex-row justify-between">
					<h2 className="text-xl font-bold my-4">
						All Accounts ({reservesInfo.length})
					</h2>
					<TabsList>
				...
				<div className="w-full flex flex-col-reverse gap-2 md:gap-0 md:flex-row justify-between my-4">
					...
					<div className="relative flex items-center">
						<Input
							...
							value={
								(columnFilters.find((f) => f.id === "symbol")
									?.value || "") as string
							}
							onChange={(e) => {
