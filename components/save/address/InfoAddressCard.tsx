...
import {
	type SavePipelineState,
	savePipelineState,
} from "@/lib/hooks/pipelines/SavePipelineState";
...
export default function InfoAddressCard() {
	...
	const userDepositBalance =
		balanceLDToken && getUserDepositBalance(reserveInfo, balanceLDToken);
	...
	const [pipeline, nextStep, resetPipeline, setPipeline] =
		useValueEffect<SavePipelineState>(savePipelineState);
	...
	const userInterest = useMemo(() => {
		const interest = getInterestAmount(userDepositBalance, userDeposit);
				...
				<div className="flex flex-col gap-2 p-6 w-full md:w-3/5">
					...
					<div className="flex flex-row justify-between items-center w-full">
						...
						{menu === "deposit" ? (
							<div className="text-md">
								...
								<div className="inline-flex flex-col items-end">
									...
									<SecurdFormat
										...
										value={
											(bigIntToDecimal(userBalance, reserveInfo.decimals) ??
												0) * coinPrice
										}
										prefix="$"
									/>
							...
							<div className="text-md">
								Account Balance
								<Help>Amount of this asset in your Savings Account</Help>
								<div className="inline-flex flex-col items-end">
									<SecurdFormat
										className="text-xl font-bold inline ml-2"
