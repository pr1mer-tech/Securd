...
export const ColorCircle = ({ colorRisk }: { colorRisk: number }) => {
	...
	return <div className={`w-6 h-6 rounded-full ${color}`} />;
};
...
export function AccountCard({
	...
	const collateralValueDecimal =
		bigIntToDecimal(collateralValue, userCollateralsInfo.decimals) || 0;
	...
	const tokensUSDPrices = getPairPrice(
		coinPrices,
		reservesInfo,
		userCollateralsInfo,
	);
	...
	const loanAUSD =
		borrowBalances?.borrowBalanceA && tokensUSDPrices.tokenA
			? borrowBalances.borrowBalanceA * tokensUSDPrices.tokenA
			: 0;
	const loanBUSD =
		borrowBalances?.borrowBalanceB && tokensUSDPrices.tokenB
			? borrowBalances.borrowBalanceB * tokensUSDPrices.tokenB
			: 0;
	...
	const borrowerCF = bigIntToDecimal(
		collateralPrice?.collateralFactor,
		userCollateralsInfo.decimals,
	);
	const balanceLT = bigIntToDecimal(
		borrowerLTs[userCollateralsInfo.addressLP],
		userCollateralsInfo.decimals,
	);
	const colorRisk =
		borrowerCF && balanceLT ? (borrowerCF / balanceLT - 1) * 100 : 0;

	return (
		<Link href={`/farm/${userCollateralsInfo.addressLP}`}>
			<Card>
				<CardContent className="pb-1">
					<div className="flex flex-row justify-between">
						...
						<div className="text-base mt-4">
							...
							<div className="font-bold inline ml-2">
								{toFormattedPercentage(totalApy, 1)}
							</div>
						</div>
					</div>
					...
					<div className="text-primary font-bold text-lg mt-4">
						Collateral
						<Help>
							Total value of locked assets backing your loans in this account
						</Help>
					</div>
					...
					<div className="text-primary font-bold text-lg mt-4">
						Loan
						<Help>Total value of loans in this account</Help>
					</div>
					<div className="flex flex-row justify-between text-base mt-2">
						{pairReservesInfosUn.reserveInfoTokenA ? (
							<div className="w-36">
					...
					<div className="flex flex-row justify-between text-base mt-4">
						<div className="text-primary font-bold text-lg">
							Liquidation Risk
							<Help>Risk of liquidation for this account</Help>
						</div>
						<Tooltip>
							<TooltipTrigger>
								<ColorCircle colorRisk={colorRisk} />
							</TooltipTrigger>
							<TooltipContent>{colorRisk.toFixed(2)}%</TooltipContent>
						</Tooltip>
					</div>
					<div className="flex flex-row justify-between text-base mt-2">
						<div className="">
							Collateral Factor
							<Help>Collateral value divided by Loan value</Help>
						</div>
						<PercentageFormat
							factor
							value={borrowerCF}
