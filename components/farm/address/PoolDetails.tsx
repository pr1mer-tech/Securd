...
import { useSaveAddressStore, useSaveAddressStoreBase } from "@/lib/data/saveAddressStore";
...
export default function PoolDetails() {
    ...
    const collateralPoolBalance = useFarmAddressStore.use.collateralPoolBalance?.();
    const collateralProportions = useFarmAddressStore.use.collateralProportions?.();
    const { totalBorrowApy, lpApy, maxLeverageApy } = useFarmAddressStore((state) => ({
    ...
    const collateralPrice = bigIntToDecimal((collateralPoolBalance ?? 0n) * (collateralProportions?.collateralPrice ?? 0n), (collateralInfo?.decimals ?? 18) * 2);
    ...
    const utilizationA = loanA / (bigIntToDecimal(lendingPoolA, collateralInfo?.decimals) ?? 0);
    ...
    return <>
        ...
        <MenuTabs className="mt-8" defaultValue="pool">
            <MenuTabsList className="h-[4.25rem]">
                <MenuTabsTrigger value="pool" className="p-8">{collateralInfo?.symbol}</MenuTabsTrigger>
                <MenuTabsTrigger value="tokenA" className="p-8">Pool {tokens[0]}</MenuTabsTrigger>
                <MenuTabsTrigger value="tokenB" className="p-8">Pool {tokens[1]}</MenuTabsTrigger>
            </MenuTabsList>
            <MenuTabsContent value="pool">
                <Card className="mt-8 p-4">
                    <div className="flex flex-row justify-evenly">
                        <Info bigIntValue={collateralPoolBalance} bigIntDecimals={collateralInfo?.decimals} value={collateralPrice} decimals={2} name="Total Supply" type="currency" tooltip="Total Collateral value for all Farmers of this LP Token" />
                        <Info value={minLT} name="Min LT" type="percentage" tooltip="Liquidation Threshold that applies when your loans value in both assets are equal" />
            ...
            <MenuTabsContent value="tokenA" className="flex flex-col md:flex-row gap-4">
                <Card className="mt-8 p-4 w-full">
                    <div className="flex flex-row justify-evenly">
                        <Info bigIntValue={lendingPoolA} bigIntDecimals={pairReservesInfos.reserveInfoTokenA?.decimals} value={
                        ...
                        <Info
                            bigIntValue={parseUnits((loanA ?? 0).toString(), pairReservesInfos.reserveInfoTokenA?.decimals ?? 18)}
                            bigIntDecimals={pairReservesInfos.reserveInfoTokenA?.decimals}
                            value={
                                (loanA ?? 0) * (coinsPrices?.[tokens[0] as keyof Coins] ?? 0)
                            }
                            name="Loans"
                            type="currency" tooltip="Total loan value for this asset" />
                    </div>
                </Card>
                <Card className="mt-8 p-4 w-full">
                    <div className="flex flex-row justify-evenly">
                        <Info value={utilizationA} name="Utilization" type="percentage" tooltip="Proportion of borrowed assets in this lending pool" />
                        <Info value={borrowPoolAPYA} name="Borrow APY" type="percentage" tooltip="Current borrowing rate for this token pair" />
                    </div>
                </Card>
            </MenuTabsContent>
            <MenuTabsContent value="tokenB" className="flex flex-col md:flex-row gap-4">
                <Card className="mt-8 p-4 w-full">
                    <div className="flex flex-row justify-evenly">
                        <Info bigIntValue={lendingPoolB} bigIntDecimals={pairReservesInfos.reserveInfoTokenB?.decimals} value={
                        ...
                        <Info
                            ...
                            type="currency" tooltip="Amount of this asset available for immediate withdrawal" />
                        <Info
                            bigIntValue={parseUnits((loanB ?? 0).toString(), pairReservesInfos.reserveInfoTokenB?.decimals ?? 18)}
                            bigIntDecimals={pairReservesInfos.reserveInfoTokenB?.decimals}
                            value={
                ...
                <Card className="mt-8 p-4 w-full">
                    <div className="flex flex-row justify-evenly">
                        <Info value={utilizationB} name="Utilization" type="percentage" tooltip="Proportion of borrowed assets in this lending pool" />
                        <Info value={borrowPoolAPYB} name="Borrow APY" type="percentage" tooltip="Current borrowing rate for this token pair" />
                    </div>
                </Card>
            </MenuTabsContent>
        </MenuTabs>
