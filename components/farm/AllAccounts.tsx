...
export const columns: ColumnDef<{
  ...
  collateralPoolBalance: bigint | undefined;
  collateralPrice: bigint | undefined;
  collateralPoolPrice: CollateralAmountPrice | undefined;
...
}>[] = [
  {
    ...
    cell: ({ row }) => (
      <PairIcon
        ...
        size="small"
        className="w-36"
        reservesInfo={row.original.reservesInfo}
      />
    ),
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const balance = bigIntToDecimal(
        ...
        row.original.collateralInfos.decimals,
      );
      const price = bigIntToDecimal(
        collateralPrice * collateralPoolBalance,
        row.original.collateralInfos.decimals * 2,
      );
      ...
      return (
        <div className="flex flex-col">
          ...
          <div className="text-sm text-secondary">${securdFormat(price)}</div>
        </div>
      );
    },
  },
  ...
  },
  {
    id: "lpApy",
    accessorFn: (row) => {
      const lpApr = row.collateralInfos.lpApr;
...
export default function AllAccounts() {
  ...
  const data = collateralInfos.map((collateralInfo) => {
    return {
      ...
      collateralPrice:
        collateralProportions[collateralInfo.addressLP]?.collateralPrice,
      ...
      <div className="mt-4 mb-16">
        <div className="flex flex-row justify-between">
          <h2 className="text-xl font-bold my-4">
            All Accounts ({collateralInfos.length})
          </h2>
          <TabsList>
        ...
        <div className="w-full flex flex-col-reverse gap-2 md:gap-0 md:flex-row justify-between my-4">
          ...
          <div className="relative flex items-center">
            <Input
              ...
              value={
                (columnFilters.find((f) => f.id === "symbol")?.value ||
                  "") as string
              }
              onChange={(e) => {
