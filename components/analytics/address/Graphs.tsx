...
export default function Graphs({ poolInfo, className }: { poolInfo: PoolDetails, className?: string }) {
    ...
    const symbol = !tokenDirection ? `${poolInfo?.token_1?.token_symbol} / ${poolInfo?.token_0?.token_symbol}` : `${poolInfo?.token_0?.token_symbol} / ${poolInfo?.token_1?.token_symbol}`;
    ...
    return <Card className="w-full pl-1 pr-4">
        <MenuTabsContent value="pool">
                ...
                <ol className="tremor-Legend-root relative overflow-hidden">
                    <div tabIndex={0} className="h-full flex flex-wrap">
                        <li className="tremor-Legend-legendItem group inline-flex items-center px-2 py-0.5 rounded-tremor-small transition whitespace-nowrap cursor-default text-tremor-content dark:text-dark-tremor-content">
                            <svg className="flex-none h-2 w-2 mr-1.5 text-[#E8A029] opacity-100" fill="currentColor" viewBox="0 0 8 8">
            ...
            <AreaChart
                ...
                data={poolInfo?.analytics?.filter((info) => info.date && info.date >= limitDate)
                    .map((info) => ({
                        ...
                        [`Price in $`]: tokenDirection
                            ? (poolInfo?.token_0?.prices?.find((price) => price.date?.toDateString() === info.date?.toDateString())?.price ?? 0)
                            : (poolInfo?.token_1?.prices?.find((price) => price.date?.toDateString() === info.date?.toDateString())?.price ?? 0),
                ...
                index="date"
                categories={[selected === "symbol" ? `Price in ${symbol}` : `Price in $`]}
                valueFormatter={(number: number) =>
                    `${selected === "dollar" ? "$" : ""}${securdFormat(number, 2)}${selected === "symbol" ? symbol.split("/")[1] : ""}`}
            ...
            <div className="flex items-center justify-end py-2">
                <ol className="tremor-Legend-root relative overflow-hidden">
                    <div tabIndex={0} className="h-full flex flex-wrap">
                        <li className="tremor-Legend-legendItem group inline-flex items-center px-2 py-0.5 rounded-tremor-small transition whitespace-nowrap cursor-default text-tremor-content dark:text-dark-tremor-content">
                            <svg className="flex-none h-2 w-2 mr-1.5 text-[#0B4B48] opacity-100" fill="currentColor" viewBox="0 0 8 8">
                                <circle cx="4" cy="4" r="4" />
                            </svg>
                            <Select value={selected} onValueChange={(v) => setSelected(v)}>
                                <SelectPrimitive.Trigger asChild>
                                ...
                                <SelectContent>
                                    <SelectItem value="symbol">Volume in {symbol.split("/")[1]}</SelectItem>
            ...
            <AreaChart
                ...
                data={poolInfo?.analytics?.filter((info) => info.date && info.date >= limitDate)
                    ...
                    .map((info) => ({
                        ...info,
                        "Volume in $": (info[`Volume in ${symbol.split("/")[1]}`] as number ?? 0) * (tokenDirection ? poolInfo?.token_1?.prices?.find((price) => price.date?.toDateString() === info.date)?.price ?? 0 : poolInfo?.token_0?.prices?.find((price) => price.date?.toDateString() === info.date)?.price ?? 0)
                    }))
                    .toReversed() ?? []}
                colors={[
                    "#0B4B48",
                ...
                index="date"
                categories={[selected === "symbol" ? `Volume in ${symbol.split("/")[1]}` : `Volume in $`]}
                valueFormatter={(number: number) =>
                    `${selected === "dollar" ? "$" : ""}${securdFormat(number, 2)}${selected === "symbol" ? symbol.split("/")[1] : ""}`}
                yAxisWidth={selected === "symbol" ? 120 : 90}
                curveType="step"
        ...
        <MenuTabsContent value="apy">
            ...
            <AreaChart
                ...
                valueFormatter={(number) => `${securdFormat(number * 100, 2)}%`}
                yAxisWidth={90}
            />
        </MenuTabsContent>
    </Card>
...
function LP_HODL({ poolInfo, limitDate, leverage }: { poolInfo: PoolDetails, limitDate: Date, leverage: number }) {
    const delay = 3 * 30
    ...
    const qToken0 = (poolInfo?.analytics?.[lastAnalytics]?.quantity_token_0 ?? 0) / (poolInfo?.analytics?.[lastAnalytics]?.quantity_token_lp ?? 0)
    const qToken1 = (poolInfo?.analytics?.[lastAnalytics]?.quantity_token_1 ?? 0) / (poolInfo?.analytics?.[lastAnalytics]?.quantity_token_lp ?? 0)

    const lastPrice0 = poolInfo?.token_0?.prices?.find((price) => price.date?.toDateString() === poolInfo?.analytics?.[lastAnalytics]?.date?.toDateString())?.price ?? 0
    const lastPrice1 = poolInfo?.token_1?.prices?.find((price) => price.date?.toDateString() === poolInfo?.analytics?.[lastAnalytics]?.date?.toDateString())?.price ?? 0

    const plp = qToken0 * lastPrice0 + qToken1 * lastPrice1
    const qty = amount / plp
    const clp = qty * plp

    const totalVolume = poolInfo?.analytics?.toReversed().map((info) => {
        const price0 = poolInfo.token_0?.prices?.find((price) => price.date?.toDateString() === info.date?.toDateString())?.price;
    ...
    const hold = poolInfo?.analytics?.toReversed().map((info) => {
        const price0 = poolInfo.token_0?.prices?.find((price) => price.date?.toDateString() === info.date?.toDateString())?.price;
        const price1 = poolInfo.token_1?.prices?.find((price) => price.date?.toDateString() === info.date?.toDateString())?.price;
    ...
    const interest = Array.from({ length: delay }, (_, i) => -amount * r * i / 365);
    const infos = poolInfo?.analytics?.filter((info) => info.date && info.date >= limitDate)?.toReversed() ?? []
